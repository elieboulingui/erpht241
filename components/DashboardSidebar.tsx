...
import { getorganisation } from "../app/listingorg/[id]/action/getorganisation"
...
const data = (orgId: string, currentPath: string) => {
  ...
  const isRootPath = currentPath === `/listingorg/${orgId}/` || currentPath === `/listingorg/${orgId}`
...
export default function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  useEffect(() => {
    const match = pathname.match(/\/listingorg\/([^/]+)/)
    ...
    if (match && match[1]) {
      ...
      const isRootPath = pathname === `/listingorg/${id}/` || pathname === `/listingorg/${id}`
      if (isRootPath) {
  ...
  return (
    <Sidebar collapsible="icon" className="border-r bg-white" variant="inset" {...props}>
      ...
      <SidebarContent className="bg-white">
        ...
        <NavMain items={sidebarData.main.filter((item) => item.title !== "Produit") as any} />
