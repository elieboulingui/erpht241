...
interface FormData {
  id: string; // ❗️ rendre obligatoire
  label: string;
  description: string;
  amount: number;
  merchantId: string;
  ...
  avatar: string;
  deadline: string;
  stepId: string; // convertir en string
}
...
export function EditDealSheets({
  deal,
  onSave,
  onOpenChange,
  stepId,
  ...
  useEffect(() => {
    if (deal) {
      setFormData({
        id: deal.id, // Utilisation de l'ID de l'opportunité existante
        label: deal.label,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (response.success && response.deal) {
      ...
      console.error("Erreur lors de la mise à jour de l'opportunité:");
    }
  };
