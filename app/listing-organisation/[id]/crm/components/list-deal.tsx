...
import { Deal, DealStag, Merchant, Contact } from "./types";
...
export function ListDeal({ merchants, contacts, deals }: ListDealProps) {
  ...
  const fetchStages = async (organisationId: string) => {
    try {
      ...
      const formattedLists: ListType[] = data.map((stage) => ({
        ...
        cards: stage.opportunities.map((opp: { id: any; label: any; description: any; amount: any; deadline: any; merchantId: any; contactId: any; tags: any; }) => ({
          id: opp.id,
          title: opp.label,
  ...
  const handleColorChange = async (
    ...
    try {
      ...
      setLists(prev =>
        prev.map(list =>
          list.id === listId ? { ...list, color: colorKey || undefined } : list
        )
      );
    } catch (e) {
    ...
    <div className="min-h-screen p-4">
      <div className="mb-4 p-3 bg-white rounded-lg shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {filters.merchant.map(merchantId => {
            const merchant = merchants.find(m => m.id === merchantId);
            return merchant ? (
              <span key={merchantId} className="inline-flex items-center rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-800">
                {merchant.name}
                <button 
                  onClick={() => handleFilterChange('merchant', filters.merchant.filter(id => id !== merchantId))}
          ...
          {filters.contact.map(contactId => {
            const contact = contacts.find(c => c.id === contactId);
            return contact ? (
              <span key={contactId} className="inline-flex items-center rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-800">
                {contact.name}
                <button 
                  onClick={() => handleFilterChange('contact', filters.contact.filter(id => id !== contactId))}
          ...
          {filters.tag.map(tag => (
            <span key={tag} className="inline-flex items-center rounded-full bg-purple-100 px-3 py-1 text-xs font-medium text-purple-800">
              {tag}
              <button 
                onClick={() => handleFilterChange('tag', filters.tag.filter(t => t !== tag))}
      ...
      {allListsEmpty && (
        <div className="text-center py-10 text-gray-500">
          Aucun deal trouvé avec les filtres actuels
        </div>
      )}
