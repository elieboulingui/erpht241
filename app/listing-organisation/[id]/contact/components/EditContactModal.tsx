...
// //   isOpen: boolean
// //   onClose: () => void
// //   onSuccess: (updatedContact: UpdatedContact) => void
// // }
...
// //       const updatedContactData = {
// //         name: formData.name,
// //         email: formData.email === "" ? null : formData.email, // Convert empty string to null
// //         phone: formData.phone,
// //         niveau: formData.niveau as "PROSPECT_POTENTIAL" | "PROSPECT" | "CLIENT",
// //         tags: tagsArray.join(),
// //         adresse: formData.adresse,
...
export function EditContactModal({ contact, isOpen, onClose, onSuccess }: EditContactModalProps) {
  ...
  useEffect(() => {
    // // Fonction pour parser les tags depuis la base de données
       ...
       const parseTags = (tags: string) => {
        try {
          ...
          if (Array.isArray(parsed)) {
            return parsed.length > 0 ? parsed.join(", ") : "Pas de tags";
          }
          return tags === "[]" ? "Pas de tags" : tags;
        } catch {
          return tags === "[]" ? "Pas de tags" : tags;
        }
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedContactData = {
        ...
        phone: formData.phone,
        niveau: formData.niveau as "PROSPECT_POTENTIAL" | "PROSPECT" | "CLIENT",
        tags: formData.tags, // On envoie la string directement, le serveur s'occupe de la conversion
        adresse: formData.adresse,
        logo: formData.logo,
        status_contact: formData.status_contact as "PERSONNE" | "COMPAGNIE" | "GROSSISTE",
        sector: formData.sector,
      ...
      const updatedContact: UpdatedContact = {
        ...
        ...updatedContactData,
        link: contact.link || `/contacts/${contact.id}`,
        tags: formData.tags, // On garde la version string pour l'affichage
      };
            ...
            <div>
              ...
              <RadioGroup
                ...
                onValueChange={(value) =>
                  setFormData({
                    ...formData,
                    status_contact: value,
                  })
                }
              >
                <div className="flex gap-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <RadioGroupItem value="PERSONNE" className="peer hidden" />
                    <div className="w-4 h-4 border-2 border-black rounded-full flex items-center justify-center">
                      <div className={`w-2.5 h-2.5 rounded-full ${formData.status_contact === "PERSONNE" ? "bg-black" : "bg-transparent"}`}></div>
                    </div>
                    Personne
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <RadioGroupItem value="COMPAGNIE" className="peer hidden" />
                    <div className="w-4 h-4 border-2 border-black rounded-full flex items-center justify-center">
                      <div className={`w-2.5 h-2.5 rounded-full ${formData.status_contact === "COMPAGNIE" ? "bg-black" : "bg-transparent"}`}></div>
                    </div>
                    Compagnie
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <RadioGroupItem value="GROSSISTE" className="peer hidden" />
                    <div className="w-4 h-4 border-2 border-black rounded-full flex items-center justify-center">
                      <div className={`w-2.5 h-2.5 rounded-full ${formData.status_contact === "GROSSISTE" ? "bg-black" : "bg-transparent"}`}></div>
                    </div>
                    Grossiste
