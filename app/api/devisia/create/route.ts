...
export async function POST(req: Request) {
    try {
      ...
      for (const product of products) {
        ...
        const taxAmount = product.unitPrice * (product.taxRate || 0); // Calcul de la taxe
        const totalPrice = product.unitPrice * product.quantity; // Calcul du prix total sans taxe
      ...
      const devis = await prisma.devis.create({
        data: {
          ...
          contactId,
          organisationId,
          createdById: session.user.id, // L'utilisateur qui a créé le devis est aussi celui qui l'a mis à jour
